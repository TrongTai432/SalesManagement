...
namespace SV20T1020091.BusinessLayers
{
    public static class OrderDataService
    {
...

        {
            ...
            || data.Status == Constants.ORDER_REJECTED) return orderDB.Delete(orderID);
        ...
        public static bool SaveAddressOrder(int orderID, string DeliveryAddress, string DeliveryProvince)
        {
            Order? data = orderDB.Get(orderID);
            if (data == null)
            {
            ...
            if (data.Status == Constants.ORDER_INIT || data.Status == Constants.ORDER_ACCEPTED)
            {
                return orderDB.SaveAddressOrder(orderID, DeliveryAddress, DeliveryProvince);
            }
            return false;
        }
    }
