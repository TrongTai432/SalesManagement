...
namespace SV20T1020091.DataLayers.SQLServer
{
    public class OrderDAL : _BaseDAL, IOrderDAL
    {
        ...
        public bool SaveAddressOrder(int orderID, string DeliveryAddress, string DeliveryProvince)
        {
        ...
        public bool DeleteDetail(int orderID, int productID)
        {
            ...
            using (var connection = OpenConnection())
            {
                var sql = @"delete from OrderDetails
                            where OrderID = @OrderID and ProductID = @ProductID"; //TODO: Hoàn chỉnh phần code còn thiếu
                var parameters = new
                {
                    OrderID = orderID,
                    ProductID = productID,
...

        {
...

            {
                ...
                var sql = @"select	od.*, p.ProductName, p.Photo, p.Unit
                            ...
                            where od.OrderID = @OrderID and od.ProductID = @ProductID"; //TODO: Hoàn chỉnh phần code còn thiếu
                var parameters = new
                {
                    OrderID = orderID,
                    ProductID = productID
        ...
        public IList<Order> List(int page = 1, int pageSize = 0, int status = 0, DateTime? fromTime = null, DateTime? toTime = null, string searchValue = "")
        {
...

            {
                var sql = @"with cte as
                            ...
                            left join Shippers as s on o.ShipperID = s.ShipperID
                            where	(@Status = 0 or o.Status = @Status)
                            and (@FromTime is null or o.OrderTime >= @FromTime) and (@ToTime is null or o.OrderTime <= @ToTime) and (@SearchValue = N''
                            or c.CustomerName like @SearchValue
                            or e.FullName like @SearchValue
                            or s.ShipperName like @SearchValue)
                            )
        ...
        {
            List<OrderDetail> list = new List<OrderDetail>(); using (var connection = OpenConnection())
            {
                var sql = @"select	od.*, p.ProductName, p.Photo, p.Unit
                            from	OrderDetails as od
                            join Products as p on od.ProductID = p.ProductID
        ...
        public bool SaveDetail(int orderID, int productID, int quantity, decimal salePrice)
        {
            ...
            using (var connection = OpenConnection())
            {
                var sql = @"if exists(select * from OrderDetails
                            ...
                            SalePrice = @SalePrice
                            where OrderID = @OrderID and ProductID = @ProductID else
